...
* `+cargo-zigbuild+` is a Rust CLI that builds Rust projects using Zig
as the linker.
...
* Install Zig (or the `+ziglang+` Python package) if you need to run
`+cargo zigbuild+`.
...
`+cargo run zigbuild --manifest-path tests/<fixture>/Cargo.toml --target <target>+`
...
* Some functionality depends on external tooling (Zig, SDKs). Call out
any required environment setup in your change summary.
